...
public class StreamLabService {
    ...
    public List<Product> RProblemTwo()
    {
        ...
        // Return the list
        return products.findAll().stream().filter(e -> e.getPrice() <= 100).toList();
    }
    ...
    public List<Product> RProblemThree()
    {
        ...
        // Return the list
    	return products.findAll().stream().filter(e -> e.getName().contains("s") || e.getName().contains("S")).toList();
    }
    ...
    public List<User> RProblemFour()
    {
        ...
        return users.findAll().stream().filter((e) -> e.getRegistrationDate().before(date)).toList();
    }
